...
- New Align tools: Line makes a tidy row - straight line, equal gaps and all props rotated the same way; Spaced does the same but leaves rotations untouched; Circle arranges the selection evenly on a circle. The gap stepper sets exact meters, and [ and ] keys (rebindable) or the stepper resize the layout live afterwards
